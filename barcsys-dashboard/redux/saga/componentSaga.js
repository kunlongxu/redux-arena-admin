import {
  FRAME_UPDATE_REFRESH, FRAME_UPDATE_NOREFRESH, FRAME_NAV_DIALOG, FRAME_SNACKBAR_SHOW,
    ...
    if (messageQueue.length > 0) {
      ...
      if (curSnackbar.showFlag === false) {
        if (curSnackbar.closeTime == null) {
          console.error("Closed snackbar has no closeTime, it should not happen! Wtf?");
        ...
        yield put({ type: FRAME_UPDATE_REFRESH, state: { snackbar: snackBarAction } })
      }
    }
  }
}
...
function* hideSnackbar() {
  ...
  if (curSnackbar.showFlag === true) {
    ...
    yield put({
      type: FRAME_UPDATE_REFRESH, state: {
        snackbar:
        { showFlag: false, message: '', closeTime: new Date().getTime() }
      }
    });
...
function* cancelDelayFunc() {
  ...
  if (curSnackbar.showFlag === true) {
    if (curSnackbar.delayFunc != null) {
      ...
      if (curSnackbar.failCb != null) { curSnackbar.failCb() }
      yield put({
        type: FRAME_UPDATE_REFRESH,
